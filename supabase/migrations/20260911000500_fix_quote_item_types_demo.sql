...
begin
  ...
  if not exists(select 1 from public.profiles where id=v_user and active=true and role in ('admin','atendimento')) then raise exception 'Usuário sem permissão para criar orçamento'; end if;
  ...
  for v_item in select value from jsonb_array_elements(coalesce(p_parts,'[]'::jsonb)) loop
    if length(trim(coalesce(v_item->>'description',''))) > 0 then
      if nullif(v_item->>'inventory_item_id','') is not null and not exists(select 1 from public.inventory_items where id=(v_item->>'inventory_item_id')::uuid and active=true) then raise exception 'Peça de estoque inválida: %',trim(v_item->>'description'); end if;
  ...
  if not exists(select 1 from public.quote_items where quote_id=v_quote_id) then raise exception 'Informe ao menos uma peça ou serviço'; end if;
...
begin
  if v_user is null then raise exception 'Usuário não autenticado'; end if;
  if not exists(select 1 from public.profiles where id=v_user and active=true and role in ('admin','atendimento')) then raise exception 'Usuário sem permissão para converter orçamento'; end if;
  ...
  for v_item in select * from public.quote_items where quote_id=p_quote_id order by created_at,id loop
    if v_item.item_type='part' then
      if v_item.inventory_item_id is not null then
        ...
        insert into public.work_order_parts(work_order_id,description,quantity,unit_cost,unit_price) values(v_work_order_id,v_item.description,v_item.quantity,0,v_item.unit_price);
      end if;
    else
      insert into public.work_order_services(work_order_id,description,quantity,unit_price,cost_price,status) values(v_work_order_id,v_item.description,v_item.quantity,v_item.unit_price,0,'pendente');
    end if;
  end loop;
