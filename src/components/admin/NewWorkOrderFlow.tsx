...
export function NewWorkOrderFlow({ onCreated }: Props) {
  ...
  if (success) return <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6"><div className="flex gap-4"><div className="rounded-2xl bg-emerald-600 p-3 text-white"><CheckCircle2 className="size-6" /></div><div><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-700">OS criada</p><h3 className="mt-1 text-2xl font-black">OS #{success.orderNumber}</h3><p className="mt-2 text-sm text-black/55">Peças e mão de obra registradas. Total inicial: <strong>{money(success.total)}</strong>.</p>{success.photoWarnings.length > 0 && <p className="mt-2 text-xs font-semibold text-amber-700">{success.photoWarnings.length} foto(s) precisam ser reenviadas.</p>}<button type="button" onClick={() => setSuccess(null)} className="goldButton mt-4">Abrir outra OS</button></div></div></div>;
...
function ModeSwitch({ value, onChange, existingLabel, newLabel, disableExisting = false }: { value: "new" | "existing"; onChange: (value: "new" | "existing") => void; existingLabel: string; newLabel: string; disableExisting?: boolean }) {
  return <div className="inline-flex rounded-xl bg-black/5 p-1"><button type="button" disabled={disableExisting} onClick={() => onChange("existing")} className={`rounded-lg px-3 py-2 text-xs font-black ${value === "existing" ? "bg-white text-[#8A5F00] shadow-sm" : "text-black/45"} disabled:opacity-35`}>{existingLabel}</button><button type="button" onClick={() => onChange("new")} className={`rounded-lg px-3 py-2 text-xs font-black ${value === "new" ? "bg-white text-[#8A5F00] shadow-sm" : "text-black/45"}`}>{newLabel}</button></div>;
}
