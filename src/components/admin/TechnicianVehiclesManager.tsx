...
export function TechnicianVehiclesManager({ onOpenOrder }: { onOpenOrder: (id: string) => void }) {
  ...
  return <div className="space-y-5">
    <div className="rounded-2xl border border-[#F0B323]/20 bg-[#F0B323]/8 p-4 text-sm text-[#6F4B00]"><b>Modo técnico.</b> São exibidos somente os dados necessários para execução e histórico do veículo. Dados pessoais completos do cliente e detalhes da agenda ficam restritos ao atendimento/administrador.</div>
...
async function openHistory(vehicle:any,setHistory:(v:any)=>void){
  const {data}=await db.from("work_orders").select("id,order_number,status,entry_at,promised_at,mileage_in,customer_report,diagnosis").eq("vehicle_id",vehicle.id).order("created_at",{ascending:false}).limit(40);
  setHistory({vehicle,orders:data??[]});
}

function HistoryModal({data,onClose,onOpenOrder}:{data:any;onClose:()=>void;onOpenOrder:(id:string)=>void}){const v=data.vehicle;return <div className="fixed inset-0 z-[80] overflow-y-auto bg-black/65 p-4"><div className="mx-auto my-8 max-w-4xl rounded-3xl bg-white p-6"><div className="flex justify-between"><div><p className="text-xs font-black uppercase tracking-widest text-[#B97900]">Histórico técnico</p><h2 className="text-2xl font-black">{v.brand} {v.model} • {v.plate}</h2></div><button onClick={onClose}><X/></button></div><div className="mt-5 space-y-5"><div className="grid gap-3 sm:grid-cols-4"><Mini label="OS" value={String(data.orders.length)}/><Mini label="Agenda" value={String(v.appointment_count??0)}/><Mini label="Ano" value={String(v.model_year??v.year??"—")}/><Mini label="Combustível" value={String(v.fuel??"—")}/></div><div className="rounded-xl bg-black/[.025] p-4 text-sm"><p><b>VIN/Chassi:</b> {v.vin||"—"}</p><p className="mt-1"><b>Quilometragem atual:</b> {v.current_mileage?`${Number(v.current_mileage).toLocaleString("pt-BR")} km`:"—"}</p><p className="mt-1"><b>Observações técnicas:</b> {v.notes||"—"}</p></div><div><h4 className="font-black">Ordens de serviço</h4><div className="mt-2 space-y-2">{data.orders.map((o:any)=><button key={o.id} onClick={()=>onOpenOrder(o.id)} className="flex w-full flex-col gap-2 rounded-xl border border-black/8 p-3 text-left sm:flex-row sm:items-center"><span className="font-black text-[#8A5F00]">OS #{o.order_number}</span><span className="flex-1 text-xs">{STATUS_LABEL[o.status as OrderStatus]??o.status}</span><span className="text-xs text-black/40">{o.entry_at?new Date(o.entry_at).toLocaleDateString("pt-BR"):"—"}</span></button>)}</div></div><p className="rounded-xl bg-black/[.025] p-3 text-xs text-black/45">Detalhes de agendamentos ficam disponíveis somente para atendimento/administrador.</p></div></div></div>}
